...
[
  ...
  explored?
  ;; used to create pairs of turtles in the make-turtles procedure
  ;; (saves deep loops and crashes)
  new?
]
...
to make-turtles
  repeat num-nodes / 2 [
    create-turtles 2 [ set size 3  set new? true]
    ask one-of turtles with [new? = true] [
      create-link-with one-of other turtles with [new? = true]
    ]
...
to pair-turtles
  while [any? turtles with [any? my-links = false]] [
    let node1 one-of turtles with [any? my-links = false]
    let node2 one-of turtles with [any? my-links = false]
    ask node1 [
...
to go
  ...
  if giant-component-size >= num-nodes [
    display
    user-message "All nodes are connected to the giant component"
    stop
  ]
  add-edge
  find-all-components
...
"connected-ish" 1.0 0 -7500403 true "plot-pen-up\nplotxy 0 0.5\nplot-pen-down\nplotxy 3 0.5" ""
